# -*- coding: utf-8 -*-
"""Foreign Exchange rates 3 .ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1aN-M7ZVUHDQcM8wSmzRJ567unqydv8hl

I have taken last 365 days data to predict next 30 days data
"""
...
sample_df.iloc[[24]]['date']

l = sample_df.iloc[[24]]['date'].dt.date
l = sample_df.iloc[[24]]['date'].index
l
...
# Changing optimizer learning rate 
optimizer = tf.keras.optimizers.RMSprop(0.001)
optimizer.learning_rate.assign(0.02)
print(optimizer.learning_rate)
...
l = sample_df.iloc[[24]]['date'].dt.date
l = sample_df.iloc[[24+365]]['date'].index
l = str(l)
l

l = sample_df.iloc[[24+395]]['date'].index
l = str(l)
l

#Taking a random point and plotting the actual and predicted data for all 22 countries 

index = 240    #index no for which model is predicting 
...
model_lstm = Sequential()
model_lstm.add(LSTM ((200) , batch_input_shape = (None, 22,365 ) , return_sequences=True ))
model_lstm.add(LSTM ((100) ,activation='sigmoid', return_sequences=True ))
model_lstm.add(LSTM ((30) ,activation='sigmoid',return_sequences=True ))
...
model_lstm = Sequential()
model_lstm.add(LSTM ((200) , batch_input_shape = (None, 22,365 ) , return_sequences=True ))
model_lstm.add(LSTM ((100) ,activation='tanh', return_sequences=True ))
model_lstm.add(LSTM ((30) ,activation='tanh',return_sequences=True ))
...
model_RNN.add(SimpleRNN ((200) , batch_input_shape = (None, 22,365 ) , return_sequences=True ))
model_RNN.add(SimpleRNN ((30) , return_sequences=True ))
...
#training model 80% data and validating on 20% data 
history = model_RNN.fit(train_x[: round(0.8 * (len(train_x)))] ,
                    train_y[: round(0.8 * (len(train_x))) ] ,
                    epochs = 300
                    , validation_data = (train_x[round(0.8 * (len(train_x))) : ] ,
                                                  train_y[ round(0.8 * (len(train_x))) : ]))
...
print(optimizer.learning_rate)
              ...
              #MODEL
model_RNN = Sequential()
model_RNN.add(SimpleRNN ((100) , batch_input_shape = (None, 22,365 ) , return_sequences=True ))
model_RNN.add(SimpleRNN ((30) , return_sequences=True ))
model_RNN.compile(loss = 'mse' , 
              optimizer = optimizer,
              metrics=['mae','mse'] )
...
predicted = model_RNN.predict(train_x)
#Taking a random point and plotting the actual and predicted data for all 22 countries 
...
predicted_values  = model_RNN.predict(testdata_x)    #predicting values for testdata 
predicted_values = predicted_values.reshape(predicted_values.shape[0] , predicted_values.shape[2] ,predicted_values.shape[1])   
                                                               #reshaping predicted values for convenience
...
model_gru.add(tf.keras.layers.GRU(200 , return_sequences=True , input_shape = (22 ,365)))
model_gru.add(tf.keras.layers.GRU(units = 30 , return_sequences=True ))

model_gru.compile(loss = 'mse', optimizer = optimizer , metrics = ['mae' , 'mse'])
model_gru.summary()
...
#training model 80% data and validating on 20% data 
history = model_gru.fit(train_x[: round(0.8 * (len(train_x)))] ,
                    train_y[: round(0.8 * (len(train_x))) ] ,
                    epochs = 300
                    ,callbacks=[early] 
                    , validation_data = (train_x[round(0.8 * (len(train_x))) : ] ,
...
model_gru = tf.keras.Sequential()
model_gru.add(tf.keras.layers.GRU(100 , return_sequences=True , input_shape = (22 ,365)))
model_gru.add(tf.keras.layers.GRU(units = 30 , return_sequences=True ))
...
model_gru = tf.keras.Sequential()
model_gru.add(tf.keras.layers.GRU(200 , return_sequences=True , input_shape = (22 ,365)))
model_gru.add(tf.keras.layers.GRU(units = 100 ,return_sequences=True ))
model_gru.add(tf.keras.layers.GRU(units = 30 ,return_sequences=True ))
...
predicted = model_gru.predict(train_x)

#Taking a random point and plotting the actual and predicted data for all 22 countries 

index = 10      #index no for which model is predicting 

testdata_x = scaledsample[index : index+365 , :]
testdata_x = testdata_x.reshape(1, testdata_x.shape[1] , testdata_x.shape[0])
testdata_y = scaledsample[index+365 : index+395]

predicted_values  = model_gru.predict(testdata_x)    #predicting values for testdata 
predicted_values = predicted_values.reshape(predicted_values.shape[0] , predicted_values.shape[2] ,predicted_values.shape[1])   
                                                               #reshaping predicted values for convenience
...
for i in range(0,22):
  ...
  plt.subplot(22,2,count);
  col = i
  plt.plot(range(0,395),sample[index+365:index+395,col], 'g')               
  plt.plot(range(365,395),result[:,col], 'r')
  plt.title(column_names[col]) 
  count = count+1;
...
model_gru = tf.keras.Sequential()
model_gru.add(tf.keras.layers.GRU(200 , return_sequences=True , input_shape = (22 ,365)))
model_gru.add(tf.keras.layers.GRU(units = 100 ,activation = 'relu' , return_sequences=True ))
model_gru.add(tf.keras.layers.GRU(units = 30 ,activation = 'relu' ,  return_sequences=True ))
...
model_gru = tf.keras.Sequential()
model_gru.add(tf.keras.layers.GRU(200 , return_sequences=True , input_shape = (22 ,365)))
model_gru.add(tf.keras.layers.GRU(units = 100 ,activation = 'sigmoid' , return_sequences=True ))
model_gru.add(tf.keras.layers.GRU(units = 30 ,activation =  'sigmoid' ,  return_sequences=True ))
...
model_gru = tf.keras.Sequential()
model_gru.add(tf.keras.layers.GRU(200 , return_sequences=True , input_shape = (22 ,365)))
model_gru.add(tf.keras.layers.GRU(units = 100 ,activation = 'tanh' , return_sequences=True ))
model_gru.add(tf.keras.layers.GRU(units = 30 ,activation =  'tanh' ,  return_sequences=True ))
